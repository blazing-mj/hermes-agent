...
def build_validator_prompt(*, contract: dict[str, Any], handoff: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "BOUNCE if proof is missing, the declared definition_of_done is not demonstrated by proof, a contract assertion is unverified, scope drift appears, the worker handoff turned the human gate OFF (handoff.human_gate_required is not true), or auto-Done/dispatch is attempted.",
            "NOTE: a CONTRACT with human_gate_required=false is a normal reversible ticket — that alone is NEVER a bounce reason. Only the worker clearing the gate in its HANDOFF is.",
