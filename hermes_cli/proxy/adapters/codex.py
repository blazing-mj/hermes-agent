...
class OpenAICodexAdapter(UpstreamAdapter):
        ...
        *,
        ...
        with self._lock:
            ...
            if retry_cred.bearer == failed_credential.bearer:
                return None
            logger.info("proxy: Codex upstream returned %s; retrying with refreshed token", status_code)
            return retry_cred
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        return UpstreamCredential(
            ...
            extra_headers=tuple(sorted(_codex_cloudflare_headers(bearer).items())),
        )
